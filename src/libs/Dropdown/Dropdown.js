...
class CustDropdown extends Component {
    render() {
        ...
        const dropdownItems = arr.map((ele, index) => {
            return <Dropdown.Item key={index} eventKey={ele.id}>{ele.name}</Dropdown.Item>
        })
        return (
            <Dropdown onSelect={(evt, evtKey) => this.props.onSelectDelegate(evt, evtKey)} className="d-xl-block d-lg-block d-md-block d-sm-block d-sx-none d-none w-100">
